...
def calc(arr, BIG_N, BIG_ROWS, BIG_COLUMNS, ROWS, COLS, fixed, isSym):
    DIM = 5
                ...
                for r in range(ROWS):
                    for c in range(COLS):
                        ...
                        for _ in permutations(SAMPLE_DIM):
                            ...
                            if bad: continue
                            mult = 1
                            if not isSym and cnt_inversion(_) % 2 != 0:
                                mult = -1
...
def get_res(
        test: list,  # matrix
        BIG_N: int,  # superlayer horizontal
        BIG_ROWS: int,  # layer vertical
        BIG_COLUMNS: int,  # layer horizontal
        ROWS: int,
        COLS: int,
        FIXED: list,  # list boolean -> fixed[i] = true <=> immutable i
        isSym: bool  # true if sym else alt
) -> list:
    if BIG_N == 1:
        FIXED[0] = True
    if BIG_ROWS == 1:
    ...
    return to_ans(calc(test, BIG_N, BIG_ROWS, BIG_COLUMNS, ROWS, COLS, FIXED, isSym), BIG_N, BIG_ROWS, BIG_COLUMNS, ROWS,
                      COLS)


test = [[4, 5, -4, -4, 3, 6, -1, 5, -1], [0, 1, -3, 0, -4, 0, 1, -4, -1], [6, 5, 5, 0, -5, -6, 5, 0, 5],
        [6, 6, 4, 5, -5, 1, 1, 1, 2], [4, 0, -4, -5, -3, 3, 3, -5, 3], [-5, -2, 1, -2, 1, 0, -4, -5, -4],
        [1, 3, -4, 3, -3, -2, 4, 2, -1], [0, 4, 1, 6, -5, 3, -4, 3, 1], [-4, -1, -5, 1, 4, -3, 3, -3, -1]]
BIG_N = 1
BIG_ROWS = 3
BIG_COLUMNS = 3
ROWS = 3
